#!/Users/amir/miniconda3/envs/plant_row/bin/python
'''
...
if not cfg.IMGNET_MODEL_PRESENCE:
    ...
    IMGNET_DATALOADER = DataLoader(IMGNET_TRAINING_DATA, batch_size=cfg.IMGNET_BATCH_SIZE, shuffle=True)
    ...
    for epoch_idx in range(cfg.IMGNET_TOTAL_EPOCH):
        ...
        for i, sample in tqdm(enumerate(IMGNET_DATALOADER)):
            ...
            batch_x, batch_y = sample["image"].to(cfg.DEVICE), sample["label"].type(torch.long).to(cfg.DEVICE)
...
#Transfer Learning
IMGNET_MODELLOAD = torch.load(cfg.IMGNET_MODEL_SAVE_PATH_FOLDER+cfg.IMGNET_MODEL_SAVE_NAME, map_location=cfg.DEVICE)
...
if os.path.exists(cfg.TRAINED_MODEL_PATH_FOLDER+cfg.TRAINED_MODEL_NAME):
    ...
    YOLO_PARAMS = torch.load(cfg.TRAINED_MODEL_PATH_FOLDER+cfg.TRAINED_MODEL_NAME, map_location=cfg.DEVICE)
...
for epoch_idx in range(cfg.TOTAL_EPOCH):
    ...
    dataloader = DataLoader(training_data, batch_size=cfg.BATCH_SIZE, shuffle=True)
    ...
    for i, sample in tqdm(enumerate(dataloader)):
        ...
        center_loss, size_loss, objectness_loss, wrong_objectness_loss = loss(predicted_array=outputs, label_array=batch_y)
        ...
        all_training_losses.append(torch.tensor([center_loss, size_loss, objectness_loss, wrong_objectness_loss]))
        total_loss.backward()
        OPTIMIZER.step()
    ...
    all_training_losses = np.average(all_training_losses, axis=0).tolist()
    ALL_TRAINING_LOSSES_LIST.append(all_training_losses)
    ...
    ALL_LOSS_FILE = open("all_loss.txt", "w")
    ALL_LOSS_FILE.write(str(ALL_TRAINING_LOSSES_LIST))
    ALL_LOSS_FILE.close()
    ...
    # Iterate over keys and get corresponding values
    ANCHOR_DATA = [(key.decode('utf-8'), YOLO_DB[key].decode('utf-8')) for key in YOLO_DB.keys()]

    # Write the anchor data to the file
